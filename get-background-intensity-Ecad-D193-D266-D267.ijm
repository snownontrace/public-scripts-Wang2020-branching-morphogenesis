...
inputFolder = "/Volumes/ShaoheGtech2/2019-DLD-1-and-L-engineering-and-characterization/191220-IF-DLD-1-cells-E-cadherin-integrin/D193-D266-D267-Ecad-MIP-for-background-estimation/";
print(inputFolder);
...
print(f, "file_name" + "\t" + "mean_BG_intensity_NLS_FP" + "\t" + "mean_BG_intensity_Ecad");
...
for (i=0; i<flist.length; i++) {
	if ( endsWith(flist[i], ".nd2") || endsWith(flist[i], ".czi") || endsWith(flist[i], ".tif") ) {
		...
		print(f, outputPrefix + "\t" + mean_BG_intensity_NLS_FP + "\t" + mean_BG_intensity_Ecad);
...
function adjustImageConstrast(id) {
	selectImage(id);
	Stack.setChannel(1); run("Yellow"); satu=2; run("Enhance Contrast", "saturated="+satu);
	Stack.setChannel(2); run("Cyan"); satu=2; run("Enhance Contrast", "saturated="+satu);
	run("Make Composite");
	return id;
}
