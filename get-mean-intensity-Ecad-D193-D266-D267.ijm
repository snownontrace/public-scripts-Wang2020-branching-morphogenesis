...
//print(inputFolder);
inputFolder = "/Volumes/ShaoheGtech2/2019-DLD-1-and-L-engineering-and-characterization/191220-IF-DLD-1-cells-E-cadherin-integrin/D193-D266-D267-Ecad-MIP-for-quantification/";
...
print(f, "file_name" + "\t" + "mean_intensity_NLS_FP" + "\t" + "mean_intensity_Ecad");
...
for (i=0; i<flist.length; i++) {
	if ( endsWith(flist[i], ".nd2") || endsWith(flist[i], ".czi") || endsWith(flist[i], ".tif") ) {
		...
		mean_intensity_NLS_FP = getResult("Mean", 0);
		mean_intensity_Ecad = getResult("Mean", 1);
		
		print(f, outputPrefix + "\t" + mean_intensity_NLS_FP + "\t" + mean_intensity_Ecad);
